#!/bin/bash
# WGS 比对脚本 - T2T (CHM13v2)
# 用法:
#   docker run -v /data:/data wgs-all align-t2t sample_id R1.fastq.gz R2.fastq.gz [threads]
#
# 输出: /data/output/{sample_id}/{sample_id}.sorted.bam + .bai
# 注意: 产出 BAM 染色体命名有 chr 前缀 (chr1/chr2/.../chrX/chrY/chrM)

set -e

SAMPLE_ID=${1:?"用法: align-t2t <sample_id> <r1.fastq.gz> <r2.fastq.gz> [threads]"}
R1=${2:?"缺少 R1 文件"}
R2=${3:?"缺少 R2 文件"}
...
# 路径
REF="/reference/t2t/genome/chm13v2.fa"
...
[ ! -f "$REF" ] && echo "错误: T2T 参考基因组不存在: $REF" && exit 1

mkdir -p "$OUTPUT_DIR" "$TMP_DIR"

echo "=========================================="
echo "  WGS 比对 (T2T / CHM13v2)"
echo "=========================================="
...
echo "  输出:     ${SORTED_BAM}"
echo "  染色体:   chr 前缀 (chr1/chr2/.../chrX/chrY/chrM)"
echo "=========================================="
echo ""
